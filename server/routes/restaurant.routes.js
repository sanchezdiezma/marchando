...
router.get("/:id", (req, res) => {
  ...
  Restaurant.findById(id)
    ...
    .catch(err => res.json({ err, errMessage: "Problema buscando un Restaurante" }))
})


router.post("/newRestaurant", (req, res) => {
  const { name, direction, description, priceRange, capacity, imageURL, typeOfKitchen, specialInfo, status } = req.body

  Restaurant.create({ name, direction, description, priceRange, capacity, imageURL, typeOfKitchen, specialInfo, status })
    .then(newRestaurant => res.json(newRestaurant))
    .catch(err => res.json({ err, errMessage: "Problema creando Restaurante" }))
})
